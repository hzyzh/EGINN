...
class GAT(torch.nn.Module):
    ...
    def forward(self, x, edge_index, nodeIDs):
        ...
        x = self.fc(x)
        # give a final graph representation
        return x
...
def train():
    ...
    for data in train_loader:
        ...
        # follow code from https://github.com/lukasruff/Deep-SVDD-PyTorch/blob/master/src/optim/deepSVDD_trainer.py#L83
        dist = torch.sum((out - center) ** 2, dim=1)
...
def test(loader):
    ...
    for data in loader:
        ...
        groundtruths.append(data.y.to(torch.int))
        allRes.append(pred)
    groundtruths = torch.concat(groundtruths, dim=0)
    allRes = torch.concat(allRes, dim=0)
    res = inteprePrediction(allRes, 5)
    corrects.append(res.eq(groundtruths))
    return res, allRes
